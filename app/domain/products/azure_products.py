...
class AzureVirtualMachine(VirtualMachine):
    ...
    def clone(self) -> 'AzureVirtualMachine':
        """
        Clona la VM de Azure, creando una nueva con las mismas configuraciones.
        
        Limpia las IPs y mantiene las configuraciones de red y grupo de recursos.
        """
        ...
        # Mantener configuraciones de red y seguridad
        # (resource_group, virtual_network, etc. se mantienen)
        
        print(f"🔄 Azure VM cloned: {self.name} -> {cloned.name}")
        return cloned
...
class AzureSQLDatabase(Database):
    ...
    def clone(self) -> 'AzureSQLDatabase':
        """
        Clona la base de datos SQL de Azure, creando una nueva instancia.
        
        Actualiza el connection string para reflejar el nuevo nombre de la base de datos.
        """
        # Usar el método base de clonación
        cloned = super().clone()
        
        # Generar nuevo connection string único para el clon
        cloned.connection_string = f"Server={cloned.server_name}.database.windows.net;Database={cloned.name}"
        
        print(f"🔄 Azure SQL Database cloned: {self.name} -> {cloned.name}")
        return cloned
...
class AzureLoadBalancer(LoadBalancer):
    ...
    def clone(self) -> 'AzureLoadBalancer':
        """
        Clona el Load Balancer de Azure, creando uno nuevo con configuraciones similares.
        
        Limpia los backend pools y genera nueva dirección IP pública.
        """
        # Usar el método base de clonación
        cloned = super().clone()
        
        # Limpiar backend pools - el clon debe configurar sus propios targets
        cloned.backend_pools = []
        cloned.frontend_ip_configs = self.frontend_ip_configs.copy()  # Mantener configuración base
        
        # Generar nueva IP pública única
        cloned.public_ip_address = f"{cloned.name}-ip-{cloned.resource_id[-8:]}"
        
        print(f"🔄 Azure Load Balancer cloned: {self.name} -> {cloned.name}")
        return cloned
