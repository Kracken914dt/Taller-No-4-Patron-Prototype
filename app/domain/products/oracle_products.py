...
class OracleComputeInstance(VirtualMachine):
    ...
    def clone(self) -> 'OracleComputeInstance':
        """Clona la instancia Oracle Compute con nueva configuración"""
        
        if not self._prepare_for_cloning():
            raise ValueError(f"Oracle Compute {self.id} no puede clonarse en estado {self.status}")
        ...
        # Rotar availability domain
        ads = ["AD-1", "AD-2", "AD-3"]
        current_ad_index = ads.index(self.availability_domain) if self.availability_domain in ads else 0
        cloned_instance.availability_domain = ads[(current_ad_index + 1) % len(ads)]
        
        # Generar nuevos OCIDs únicos
        cloned_instance.subnet_id = f"ocid1.subnet.oc1..{uuid.uuid4().hex[:20]}"
...
class OracleAutonomousDatabase(Database):
    ...
    def clone(self) -> 'OracleAutonomousDatabase':
        """Clona la base de datos Oracle Autonomous con nueva configuración"""
        
        if not self._prepare_for_cloning():
            raise ValueError(f"Oracle Autonomous DB {self.id} no puede clonarse en estado {self.status}")
        ...
        cloned_db.endpoint = f"{cloned_db.name}_{random.randint(1000, 9999)}.adb.{cloned_db.region}.oraclecloud.com"
...
class OracleLoadBalancer(LoadBalancer):
    ...
    def clone(self) -> 'OracleLoadBalancer':
        """Clona el Load Balancer Oracle con nueva configuración"""
        
        if not self._prepare_for_cloning():
            raise ValueError(f"Oracle Load Balancer {self.id} no puede clonarse en estado {self.status}")
        ...
        # Limpiar backend sets (se configurarán después del clon)
        cloned_lb.backend_sets = []
        
        # Generar nuevos subnet IDs únicos
        cloned_lb.subnet_ids = [f"ocid1.subnet.oc1..{uuid.uuid4().hex[:20]}" for _ in self.subnet_ids]
...
class OracleObjectStorage(Storage):
    ...
    def clone(self) -> 'OracleObjectStorage':
        """Clona el bucket Oracle Object Storage con nueva configuración"""
        
        if not self._prepare_for_cloning():
            raise ValueError(f"Oracle Object Storage {self.id} no puede clonarse en estado {self.status}")
