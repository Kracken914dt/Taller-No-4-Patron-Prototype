...
def create_infrastructure(request: InfrastructureCreateRequest):
    ...
    try:
        ...
        # Guardar VM en el repositorio global
        try:
            repository.save(vm)
            print(f"✅ VM guardada en repositorio: {vm.resource_id}")
        except Exception as e:
            print(f"⚠️ Error al guardar VM en repositorio: {e}")
        vm_info = {
            "name": vm.name,
        ...
        if request.include_database:
            ...
            try:
                repository.save(db)
                print(f"✅ Database guardada en repositorio: {db.resource_id}")
            except Exception as e:
                print(f"⚠️ Error al guardar Database en repositorio: {e}")
            db_info = {
                "name": db.name,
        ...
        if request.include_load_balancer:
            ...
            try:
                repository.save(lb)
                print(f"✅ LoadBalancer guardado en repositorio: {lb.resource_id}")
            except Exception as e:
                print(f"⚠️ Error al guardar LoadBalancer en repositorio: {e}")
            lb_info = {
                "name": lb.name,
        ...
        if request.include_storage:
            ...
            try:
                ...
                print(f"✅ Storage guardado en repositorio: {storage.resource_id}")
            except Exception as e:
                print(f"⚠️ Error al guardar Storage en repositorio: {e}")
            storage_info = {
                "name": storage.name,
